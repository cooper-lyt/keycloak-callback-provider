...
mvn -f ../../pom.xml clean package \
  && docker build -t keycloak-callback:snapshot ../../target/ \
  && docker save -o ../../target/keycloak-callback.tar keycloak-callback:snapshot \
  && scp -r keycloak root@$REMOTE_ADDR:$REMOTE_PATH/ \
  && scp ../../target/keycloak-callback.tar root@$REMOTE_ADDR:$REMOTE_PATH/keycloak/ \
  && rm -f ../../target/keycloak-callback.tar \
